...
const ShowAllCountries = (props) => {  
  ...
  const selectedRegoin = (e) => {
    setRegion(e);
    setCountryCard(e !== "All Region" ? props.data.filter(country => country.region === e) : props.data);
    document.getElementById("P_SelectedRegion_JSX").textContent = e;
  }

  const searchCountry = (e) => {
    if( e !== ""){
      setCountryCard(region !== "All Region" ? props.data.filter(country => country.name.toLowerCase().indexOf(e.toLowerCase()) > -1 && country.region === region) : props.data.filter(country => country.name.toLowerCase().indexOf(e.toLowerCase()) > -1));
    }
    else{
      setCountryCard(region !== "All Region" ? props.data.filter(country => country.region === region) : props.data);
    }
  }
  return (
